...
class AvS_FastSimpleImport_Model_Import_Entity_Product extends Mage_ImportExport_Model_Import_Entity_Product
{
     ...
     * @return Mage_ImportExport_Model_Import_Entity_Product
     */
    protected function _saveProductsOld()
    {
                    ...
                    foreach ($this->_imagesArrayKeys as $imageCol) {
                        if (!empty($rowData[$imageCol])) { // 5. Media gallery phase
                            if (!array_key_exists($rowData[$imageCol], $uploadedGalleryFiles)) {
                                $uploadedGalleryFiles[$rowData[$imageCol]] = $this->_uploadMediaFiles(
                                    $rowData[$imageCol]
                                );
                            }
        ...
        while ($bunch = $this->_dataSourceModel->getNextBunch()) {
            ...
            foreach ($bunch as $rowNum => $rowData) {
                ...
                if (self::SCOPE_NULL == $rowScope) {
                    // for multiselect attributes only
                    if (!is_null($previousAttributeSet)) {
                        $rowData[Mage_ImportExport_Model_Import_Entity_Product::COL_ATTR_SET]
                            = $previousAttributeSet;
                    }
                    if (is_null($productType) && !is_null($previousType)) {
                        $productType = $previousType;
                    }
                ...
                try {
                    list($attributes, $resParams) = $this->_prepareAttributes($rowData, $rowScope, $attributes, $rowSku, $rowStore);
                    $resParamsAll = array_merge($resParamsAll, $resParams);
                } catch (Exception $e) {
                    Mage::logException($e);
                    continue;
                }
            ...
            if(count($resParamsAll) > 0) {
                $this->_saveResParams($resParamsAll);
            }
        }
        if (method_exists($this, '_fixUrlKeys')) { // > EE 1.13.1.0
            $this->_fixUrlKeys();
    ...
    protected function _prepareAttributes($rowData, $rowScope, $attributes, $rowSku, $rowStore)
    {
        ...
        foreach ($rowData as $attrCode => $attrValue) {
            ...
            foreach ($storeIds as $storeId) {
                if (($attrCode == 'res_prices') || ($attrCode == 'res_excluded_dates')
                    || ($attrCode == 'res_serialnumbers')
                ) {
                    if ($attrValue) {
                ...
                if ('multiselect' == $attribute->getFrontendInput()) {
                    ...
                    // This part I'm not sure why it is.
                    //$attributes[$attrTable][$rowSku][$attrId][$storeId] = $attrValue;
                } else {
                    $attributes[$attrTable][$rowSku][$attrId][$storeId] = $attrValue;
                }
        ...
        return array($attributes, $res_params);
    }
                    ...
                    if ($attrCode == 'res_prices') {
                        ...
                        foreach ($arrValues as $arrValue) {
                            ...
                            $resModel->setEntityId($productId)
                                ...
                                ->setReservationpricesdatesId($resp)
                                ->save();
                        }
                    }
